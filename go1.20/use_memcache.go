...
func useMemcache(ctx context.Context, w io.Writer) {
...

	{
		...
		// What happens when setting/getting an empty value?
		// Set succeeds, Get succeeds, with an empty item value.
		key := "EMPTY"
		...
		assertf(len(item.Value) == 0, "expected empty value, got %q", string(item.Value))
	}
	...
	{
		// TRADITIONAL USAGE, BUT CHECK THE NEW FIELDS RETURNED (DEFAULT VALUES)
	...
	{
		// DELETE LOCK
		// this feature (included in the proto) is not available in the Go lib exported API.
	}
	...
	// Cleanup (teardown)
	keys := []string{"mykey", "mykey1", "mykey2", "EMPTY"}
